...
export const signController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const signRepository = new SignRepository(controllerInput.dbConnection, { session });
    ...
    const signUseCase = new SignUseCase({
    ...
    const response = await signUseCase.handle({
