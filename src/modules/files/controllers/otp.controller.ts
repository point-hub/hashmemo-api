...
export const otpController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const otpRepository = new OtpRepository(controllerInput.dbConnection, { session });
    ...
    const response = await otpUseCase.handle({
