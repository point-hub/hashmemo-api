...
import { RejectUseCase } from '../use-cases/reject.use-case';

export const rejectController: IController = async (controllerInput: IControllerInput) => {
  let session;
  try {
    ...
    const rejectUseCase = new RejectUseCase({
    ...
    const response = await rejectUseCase.handle({
