...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<IFolder | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<IFolder | null> {
    const response = await this.database.collection(collectionName).retrieve<IFolder>(_id, this.options);
    if (!response) {
      return null;
    }
